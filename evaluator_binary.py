...
def ece_binary(y_true, y_prob, n_bins=15):
    ...
    y_prob = np.asarray(y_prob).astype(float).clip(1e-7, 1-1e-7)  # ← clip
    bins = np.linspace(0, 1, n_bins+1); ece = 0.0
    for i in range(n_bins):
        if i < n_bins-1:
            idx = (y_prob >= bins[i]) & (y_prob < bins[i+1])
        else:
            idx = (y_prob >= bins[i]) & (y_prob <= bins[i+1])     # ← include right edge
...
def compute_threshold(y_true, y_prob, method="youden"):
    ...
    # If only a single category is present, ROC cannot be calculated; fall back to 0.5.
    ...
    if method == "youden":
        ...
        tgt = float(method.split("=")[1])
        tgt = np.clip(tgt, tpr.min(), tpr.max())                  # ← clamp
        idx = np.argmin(np.abs(tpr - tgt)); return float(thr[idx])
    return 0.5
...
def evaluate_binary(y_true, y_prob, threshold=0.5):
    ...
    # These will throw errors when in a single category → set try/except to nan
    try: auroc = float(roc_auc_score(y_true, y_prob))
    except Exception: auroc = float("nan")
    try: auprc = float(average_precision_score(y_true, y_prob))
    except Exception: auprc = float("nan")
    try: ll = float(log_loss(y_true, y_prob))
    ...
    overall = dict(
        ...
        F1_macro = float(f1_score(y_true, y_pred, average="macro", zero_division=0)),      # ← zero_division
